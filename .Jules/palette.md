...
## 2026-05-20 - Modal Form Accessibility
**Learning:** Found that modals using `<div>` containers instead of `<form>` for data entry lack native validation, 'Enter-to-submit' behavior, and proper label association.
**Action:** Wrap inputs in a `<form onSubmit={...}>`, add `htmlFor`/`id` pairs, and use native `required` attributes with visual indicators to improve form accessibility and UX.
